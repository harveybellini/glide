...
| Hosted background path, first attempt | Failed. `scripts/verify_deployed_sample.py` passed steps 1-4 on 0.4.1 (201 session, first check `needs_input` with one block and one open decision, move to `completed` with two blocks, repeat all-`unchanged`) but step 5 timed out after the 900-second window. The scheduled run did arrive at 17:04:59Z, about ninety seconds after the window closed | 14 Sep |
| Root cause | The dispatcher's per-tick scan budget was 100 items a page for 10 pages, against a 3,743-item table: the cursor only swept a quarter of the table per tick, so a freshly created sample could wait twenty minutes for its first background check. Every tick had run without error and the queues were empty, which is what pointed at coverage rather than failure | 14 Sep |
